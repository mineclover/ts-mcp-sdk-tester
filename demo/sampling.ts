...
export function generateDemoSamplingResponse(
  ...
  if (messages.length > 0) {
    ...
    if (
      ...
      if (userText.toLowerCase().includes("hello")) {
        ...
        responseText = "Here's a code example that should help with your request:\n\n```javascript\nconst example = 'demo code';\nconsole.log(example);\n```";
  ...
  if (systemPrompt) {
    if (systemPrompt.toLowerCase().includes("concise")) {
      ...
      responseText += " Let me provide additional context and examples to make this more comprehensive.";
    }
  }
