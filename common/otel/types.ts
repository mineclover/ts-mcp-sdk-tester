...
import type { Attributes, Span, SpanContext, SpanStatus } from "@opentelemetry/api";
...
export interface McpTracer {
  ...
  startActiveSpan<T>(name: string, options: McpSpanOptions, fn: (span: Span) => T): T;
}
...
export interface McpOTelProvider {
  ...
  getMeter(name: string, version?: string): unknown; // Meter type from metrics API
  getLogger(name: string, version?: string): unknown; // Logger type from logs API
...
export const MCP_SPAN_NAMES = {
  ...
  TOOL_VALIDATION: (toolName: string) =>
    `mcp.tool.validate.${toolName.replace(/[^a-zA-Z0-9]/g, "_")}`,

  // Resource operations - specific resource operations
  RESOURCE_LIST: "mcp.resources.list",
  RESOURCE_READ: (uri?: string) =>
    uri ? `mcp.resource.read.${uri.split("/").pop() || "unknown"}` : "mcp.resource.read",
  RESOURCE_SUBSCRIBE: (uri?: string) =>
    uri ? `mcp.resource.subscribe.${uri.split("/").pop() || "unknown"}` : "mcp.resource.subscribe",
  RESOURCE_UNSUBSCRIBE: (uri?: string) =>
    uri
      ? `mcp.resource.unsubscribe.${uri.split("/").pop() || "unknown"}`
  ...
  COMPLETION_COMPLETE: (provider?: string) =>
    provider ? `mcp.completion.${provider}` : "mcp.completion.complete",
  ...
  BUSINESS_OPERATION: (operation: string) =>
    `mcp.business.${operation.replace(/[^a-zA-Z0-9]/g, "_")}`,

  // Testing and manual operations
  TEST_INTEGRATION: (testName: string) =>
    `mcp.test.integration.${testName.replace(/[^a-zA-Z0-9]/g, "_")}`,
  TEST_MANUAL: (operation: string) => `mcp.test.manual.${operation.replace(/[^a-zA-Z0-9]/g, "_")}`,
  TEST_ENDPOINT: (endpoint: string) =>
    `mcp.test.endpoint.${endpoint.replace(/[^a-zA-Z0-9]/g, "_")}`,
