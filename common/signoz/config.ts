...
export function createSigNozConfig(config: SigNozConfig): McpOTelConfig {
  const isCloud = config.endpoint.includes('signoz.cloud');
...
export function validateSigNozConfig(config: SigNozConfig): { valid: boolean; errors: string[] } {
  ...
  if (!config.serviceName) {
    ...
    errors.push('Service name must start with a letter and contain only alphanumeric characters, dots, dashes, and underscores');
  }
