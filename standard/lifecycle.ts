...
export class LifecycleManager {
  ...
  async shutdown(reason: string = "Server shutdown requested") {
    ...
    try {
      ...
      if (previousState === LifecycleState.INITIALIZING) {
        logger.warning(
          "Shutdown requested during initialization - will complete gracefully",
          "lifecycle"
        );
  ...
  private registerLifecycleEndpoints(server: McpServer) {
    try {
      ...
      server.setNotificationHandler(InitializedNotificationSchema, async () => {
        if (this.state === LifecycleState.SHUTTING_DOWN || this.state === LifecycleState.SHUTDOWN) {
          logger.warning(
            "Initialized notification received during shutdown - ignoring",
            "lifecycle"
          );
          return;
        }
        ...
        if (this.state === LifecycleState.INITIALIZED) {
          ...
          logger.warning(
            `Initialized notification received in invalid state: ${this.state}`,
            "lifecycle"
          );
        }
      });
      ...
      logger.warning(
        {
          message:
            "Failed to register lifecycle endpoints - MCP server may not support notification handlers",
          error: error instanceof Error ? error.message : String(error),
        },
        "lifecycle"
      );
  ...
  private setupShutdownHandlers() {
    ...
    logger.info(
      `Registered ${shutdownSignals.length} signal handlers + 2 error handlers`,
      "lifecycle"
    );
